...
на OpenBfB, конечно, будут, а по обычному, на CloseBfB - нет. И это вселяет в
меня скорбь и страдание. ;-) Кнопочка "Save all" сохраняет в телефон все, что
есть в правом окне. При этом старое содержимое ЕЕПРОМов удаляется!!! (Вернее,
удаляется EELITE, но оно не суть важно - команды для удаления EEFULL просто
нет. Точнее, есть, но она не работает.) Учитывая упомянутый факт о том, что
грузятся не все блоки я бы на вашем месте злоупотреблять "Save all" не
стал. ;-) А вот "Save selected" - это интереснее. Она сохраняет только
выделенные блоки, не удаляя при этом текущего содержимого. Вот-с... Кнопка
"Load custom" позволяет самому задать точные номера блоков для загрузки -
можно задавать несколько штук через запятую или сразу диапазон через дефис.
Еще есть такая полезная вещь как "Delete directly" - удаление блока не только
из списка, но и прямо из телефона, не трогая остальных.
...
в соответствующей менюшке - иначе он банально не забутается. ;-) Во избежание
истошных ламерских криков заблокировал в х65 кнопку "SaveAll", а то ведь
все горазды на нее жать и телефоны прибивать... Вообще, в данный момент 
поддержка х65 находится в состоянии тестирования, так что если ваш телефон
помрет - в этом нет ничего удивительного и это будут только ваши проблемы. ;-)
Но учитывая полное отсутствие какой-либо достоверной информации о протоколе
BFC в x65, и такая поддержка уже рулез. ;-))))
